...
/*   Updated: 2022/11/12 18:31:34 by jecolmou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_path_construction(t_list **cmd, t_data *x, t_list **cpenv)
{
	...
	if (!path_env)
		return (ft_close_files(x), ft_error_command_not_f(x->option[0]),
			ft_lstclear(cpenv, ft_free_words), ft_exit_bis("127", x), NULL);
	option = get_env(tmp_cmd);
	if (!option)
		return (ft_free_array(path_env),
			ft_lstclear(cpenv, ft_free_words), ft_exit_bis("127", x), NULL);
	...
	free(x->pc);
	ft_free_array(option);
	return (ft_close_files(x), ft_lstclear(cpenv, ft_free_words),
		ft_free_array(x->option), ft_free_array(path_env),
