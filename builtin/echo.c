...
/*   Updated: 2022/11/12 13:39:57 by jecolmou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_orga_n(t_list *cmdredir, char *str, t_data *x)
{
	...
	{
		x->echo = 0;
		if (((t_words *)cmd->content)->token == 9 && cmd->next)
			cmd = ft_echo_quotes(cmdredir, cmd, x);
		else if (((t_words *)cmd->content)->token == 9 && (!(cmd->next)))
			x->echo = 6;
		if (cmd->next == NULL || (!(cmd->next)) || (!(cmd->next->content)))
			x->finish = 1;
